var app = angular.module('MyApp', ['ngResource', 'ngRoute', 'ngGrid', 'cmsApp', 'validator.directive', 'ngSanitize']).config(function($routeProvider) {
  $routeProvider
    .when('/store', {
  ...
  // 20160626 20:57 created by mark<daijx@techdop.cffex.net>
  .when('/gridDesigner', {
    templateUrl: 'gridDesigner/newGridBuilder.html',
    controller: 'NewCreateCtrl'
  })
  .when('/formDesigner', {
    templateUrl: 'formDesigner/newFormBuilder.html',
    controller: 'NewCreateCtrl'
  })
});
...
app.controller('storeManage', function($scope, instance) {
...
app.controller('storeEdit', function($scope, $http, $timeout, storeInstance, setMetaInstance, smeDeploySet, smeCreateSet, spaceSchema, StatusesConstant, schemaInstance) {
  ...
  $scope.store = {
    ...
    cancelStore: function() {
      ...
      storeInstance.get({
        setName: $scope.store.entitySetName
      }, this.data); //change this.store.data to this.data
    },
    publishStore: function() {
      console.log($scope.store.data.entitySetName)
      smeDeploySet.deploy({
          spaceName: $scope.space.realname,
          setName: $scope.store.data.entitySetName
        },
        function(data) {
  ...
  $scope.store.defaultValueUntil = {
    ...
    gridOptions: { //initial grid
      ...
      columnDefs: [{
        field: "name",
        displayName: "名称",
        editableCellTemplate: this.name
      }, {
        field: "dType",
        displayName: "类型",
        editableCellTemplate: this.dType,
    ...
    saveSelectSchema: function() {
      ...
      $scope.store.data.defaultValues.unshift({
        name: null,
        dType: 1,
        defaultValue: null,
        evalExp: null
      ...
      schemaInstance.get({
        schemaName: $scope.store.set.data.entitySchemaList[index].schemaName
      }, function(data) {
      ...
      $scope.$apply(
        storeInstance.get({
          ...
          function(resolve) {
            ...
            $scope.store.isPKeyAuto = false;
            if ($scope.store.data.autoValue == $scope.store.data.pKey) {
              $scope.store.isPKeyAuto = true;
            }
            ...
            if (typeof($scope.store.data.entitySetName) == "undefined") {
              ...
              setMetaInstance.get({
                ...
                function(reject) {
                  console.log("get SetMetadata failed" + reject)
                }
              );
            }
          },
          function(reject) {
            console.log("get store failed")
          }
        ));
  ...
  var checkPKAuto = function() {
    if ($scope.store.isPKeyAuto == true) {
      if ($scope.store.pkeyType != 2) {
        $scope.store.data.autoValue = $scope.store.data.pKey;
      } else {
        alert("自增长字段不能是string类型");
      }
    } else {
      $scope.store.data.autoValue = "";
    }
  }
